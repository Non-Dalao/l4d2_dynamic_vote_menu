void LoadAnneConfig(int SpecialNum)
{
	ServerCommand("sm_weapon weapon_sniper_awp damage 350");
	ServerCommand("sm_weapon weapon_sniper_scout damage 200");
	char buffer[128];
	char buffer2[128];
	if(IsPlayerLeftCP)
	{
		Format(buffer,sizeof(buffer),"exec vote/AnneHappy%d",SpecialNum);
		ServerCommand(buffer);
	}
	Format(buffer2,sizeof(buffer2),"\x04正在使用\x03 AnneHappy%d\x04 配置",SpecialNum);
